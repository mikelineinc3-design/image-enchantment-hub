    ...
    for (let i = 0; i < apiKeys.length; i++) {
      ...
      try {
        ...
        if (!contentType.includes('application/json')) {
          ...
          console.error(`[${keyType}] Non-JSON response (content-type: ${contentType}):`, rawBody.slice(0, 300));
          lastFailure = {
            provider: keyType,
            status: 502,
            code: 'non_json_response',
            message: `${keyType.toUpperCase()} returned a non-JSON response (likely blocked by a proxy/firewall): ${rawBody.slice(0, 150).replace(/\s+/g, ' ')}`,
            retryable: true,
          };
          failures.push(lastFailure);
          continue;
